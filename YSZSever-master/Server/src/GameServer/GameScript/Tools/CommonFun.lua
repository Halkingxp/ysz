...
if SHIELD_WORD == nil then
	SHIELD_WORD =
	{
	  ...
	  "count",  "mid",  "truncate",  "declare",  "robot",  "Robot",  "xp_",  "sp_",  "-",  "*",  "\"",  "'",  "+",  ";",
	}
end
...
function SQLQuery(strSQL_Syntax, strCallback)
	...
	net:SendToDBServer(tData, PROTOCOL.SS_REQUEST_LUA)
end
...
-- 返回说明: 起始时间从1970-01-01 08:00:00
-- 返回说明: 返回值为整型 (单位毫秒)
function GetSystemTimeToMillisecond()
	local nNowTime = c_GetTimeToMillisecond()
	return nNowTime
end
...
function RegistryTimer(nInterval, iCount, nProtocol, tProtocalData)
	...
	--print("=========== RegistryTimer:", GetTimeToString(), nTimerID, nInterval, nProtocol, iCount)
	return nTimerID
end
...
function CancelTimer(nTimerID)
	--print("=========== CancelTimer:", GetTimeToString(), nTimerID)

...
#include "World/BackstageMgr.h"
#include "Http/HttpClient.h"

using namespace std;

CXml		g_cSystemXml;		// 读取系统配置
String		g_cConfigDirectory; // Csv配置文件路径
...
///////////////////////////////////////////////////////
...
#if defined (WIN32)
	String directory = "";
	configfile = "gameserver.conf";
	g_cConfigDirectory = directory + "Csv/";
#else
	...
	configfile = directory+"etc/gameserver.conf";
	g_cConfigDirectory = directory + "csv/";
	...
	sHttpClient.ClearAll();

	int iRet = 0;


	// 所有功能模块的初始化
	iRet = sWorld.Initialize(directory);
	if (0 != iRet)
	{
    ...
    command.SetServerName("GameServer");
    command.Start();

    printf("------------ 本地初始化完成 请求DB数据! ------------\r\n");
